function f=path(Link_matrix,extra_link,extra_link_flow,node1,node2)
 ...
 c=1;
 recursive(r1,c1,d_matrix,node1,node2,extra_link_flow,t,path,c,Link_matrix,extra_link);
end
function f= recursive(i,j,d_matrix,node1,node2,flow_matrix,t,path,cnt,Link_matrix,extra_link)
r1=0;
r2=0;
c1=0;
c2=0;
...
if(r1==node1)
  ...
  thetacalculate(node1,node2,path,flow_matrix,Link_matrix,extra_link);
  return;
elseif (c1== node1)
...
if(r2==node1)
  path(cnt,1)=j; path(cnt,2)=r2;  thetacalculate(node1,node2,path,flow_matrix,Link_matrix,extra_link); 
  return;
elseif (c2 == node1)
  path(cnt,1)=c2; path(cnt,2)=j; thetacalculate(node1,node2,path,flow_matrix,Link_matrix,extra_link);
  return;
% elseif(r2~=0 && c2~=0)
%     [r2,c2]=exceptional(r2,c2,d_matrix); 
 ...
 if(r1~=0)
    f= recursive(r1,0,d_matrix,node1,node2,flow_matrix,t,path,cnt,Link_matrix,extra_link);

 elseif(c1~=0)
    f= recursive(c1,0,d_matrix,node1,node2,flow_matrix,t,path,cnt,Link_matrix,extra_link);
 elseif(c2~=0)
    f= recursive(c2,0,d_matrix,node1,node2,flow_matrix,t,path,cnt,Link_matrix,extra_link);
  elseif(r2~=0)
    f= recursive(r2,0,d_matrix,node1,node2,flow_matrix,t,path,cnt,Link_matrix,extra_link);
 end
 end
